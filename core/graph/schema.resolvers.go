...
// CreateSQLDataExport is the resolver for the CreateSQLDataExport field.
func (r *mutationResolver) CreateSQLDataExport(ctx context.Context, input model.SQLDataExportInput) (*model.ExportDownload, error) {
	plugin, config := GetPluginForContext(ctx)
	if err := ValidateStorageUnit(plugin, config, input.Schema, input.StorageUnit); err != nil {
		return nil, err
	}
	if err := validateSQLDataExportTable(plugin, config, input.Schema, input.StorageUnit); err != nil {
		return nil, err
	}

	req := &engine.SQLDataExportRequest{
		Schema:      input.Schema,
		StorageUnit: input.StorageUnit,
		Mode:        engine.SQLDataExportMode(input.Mode.String()),
		Where:       input.Where,
		Sort:        input.Sort,
		Limit:       input.Limit,
	}
	exportFile, err := createSQLDataExportFile(plugin, config, req)
	if err != nil {
		log.WithFields(log.Fields{
			"operation":    "CreateSQLDataExport",
			"schema":       input.Schema,
			"storage_unit": input.StorageUnit,
			"mode":         input.Mode.String(),
		}).WithError(err).Error("SQL data export failed")
		return nil, err
	}

	return &model.ExportDownload{
		ID:          exportFile.ID,
		Filename:    exportFile.Filename,
		ContentType: exportFile.ContentType,
		DownloadURL: "/api/exports/" + exportFile.ID + "/download",
		Size:        int(exportFile.Size),
	}, nil
}
...
func (r *mutationResolver) ImportPreview(ctx context.Context, input model.ImportPreviewInput) (*model.ImportPreview, error) {
	data, err := readUploadBytes(input.File, maxImportFileSizeBytes)
	if err != nil {
		return nil, err
	}

	result, err := parseImportFile(data, input.Options, importPreviewRowLimit, false)
	if err != nil {
		return nil, err
	}
	...
	case model.ImportTargetModeExistingTable:
		if input.ExistingTable == nil || input.NewTable != nil {
			return nil, newImportValidationError(importValidationTargetModeInvalid)
		}
		target := input.ExistingTable
		...
		allowAutoGenerated := target.UseHeaderMapping
		mappings, autoGeneratedColumns, err := buildImportMappingInputs(
			preview.Columns,
			columns,
			target.UseHeaderMapping,
			allowAutoGenerated,
		)
		if err != nil {
			...
			if len(autoGeneratedColumns) > 0 {
				...
				if len(mappings) > 0 {
					preview.Mapping = importMappingPreviewItems(mappings)
				}
			}
			return preview, nil
		}
	...
	case model.ImportTargetModeNewTable:
		if input.NewTable == nil || input.ExistingTable != nil {
			return nil, newImportValidationError(importValidationTargetModeInvalid)
		}
		preview.NewTable = buildNewTablePreview(input.File.Filename, result.columns, plugin.GetDatabaseMetadata())
	default:
		return nil, newImportValidationError(importValidationTargetModeInvalid)
	}
...
func (r *mutationResolver) ImportTableFile(ctx context.Context, input model.ImportFileInput) (*model.ImportResult, error) {
	...
	case model.ImportTargetModeExistingTable:
		if input.ExistingTable == nil || input.NewTable != nil {
			return importResult(false, importValidationTargetModeInvalid), nil
		}
		return importExistingTableFile(plugin, config, parsed, input.ExistingTable)
	case model.ImportTargetModeNewTable:
		if input.NewTable == nil || input.ExistingTable != nil {
			return importResult(false, importValidationTargetModeInvalid), nil
		}
		return importNewTableFile(plugin, config, parsed, input.NewTable)
	default:
		return importResult(false, importValidationTargetModeInvalid), nil
	}
}
