      ...
      <div className="rounded-md border">
        <table className="w-full text-sm">
          <thead className="bg-muted/50 text-xs uppercase text-muted-foreground">
            <tr>
              <th className="px-4 py-2 text-left font-medium">{t('redis.key.hashFieldPlaceholder')}</th>
              <th className="px-4 py-2 text-left font-medium">{t('redis.key.hashValuePlaceholder')}</th>
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        variant="ghost"
                        size="icon-sm"
                        onClick={() => setHashPairs(draft.hashPairs.filter((_, i) => i !== index))}
                        disabled={state.isSubmitting}
                        className={`text-muted-foreground hover:text-destructive transition-opacity ${draft.hashPairs.length > 1 ? 'opacity-0 group-hover:opacity-100' : 'invisible'}`}
                      >
