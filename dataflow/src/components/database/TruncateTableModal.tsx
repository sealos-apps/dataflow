...
export function TruncateTableModal({ isOpen, onClose, connectionId, databaseName, tableName, onSuccess }: TruncateTableModalProps) {
    ...
    const handleTruncate = async () => {
        ...
        try {
            const response = await fetch('/api/connections/truncate-table', {
                method: 'POST',
            ...
            if (data.success) {
                setResult({ success: true, message: `Table "${tableName}" has been truncated successfully.` });
                if (onSuccess) onSuccess();
            } else {
                setResult({ success: false, message: data.error || "Failed to truncate table" });
            }
        } catch (error: any) {
                    ...
                    {!result ? (
                        <>
                            <div className="rounded-lg bg-red-50 p-4 text-sm text-red-800 border border-red-100">
                                <p className="font-medium">Warning: This will remove ALL data and reset auto-increment counters.</p>
                                <p className="mt-1">
                                    Are you sure you want to truncate table <strong>{tableName}</strong>? This action cannot be undone.
                                </p>
                            </div>
                        </>
                    ) : (
